...
Contexte Visuel : Imaginons un lanceur de fléchettes visant le centre d'une cible à l'origine $(0,0)$. À cause de multiples facteurs aléatoires inhérents à son geste, les impacts forment un nuage de points dispersés. Le graphique ci-dessous illustre cette dispersion. On s'intéresse à la probabilité qu'une fléchette atterrisse dans une très petite zone, notée $dA$, autour d'un point $(x,y)$.
...
xlabel=$x$,
ylabel=$y$,
axis line style={magenta},
xlabel style={anchor=west, magenta},
ylabel style={anchor=south, magenta},
xmin=-3.5, xmax=3.5,
ymin=-3.5, ymax=3.5,
tick label style={font=\tiny}
]
...
Objectif : Démontrer que la célèbre courbe en cloche (la loi normale) n'est pas arbitraire, mais la conséquence mathématique inévitable de deux principes physiques simples sur le hasard.

1. Le Point de Départ : Densité et Aire $dA$
Dans le monde continu, la probabilité de toucher exactement le point géométrique $(x,y)$ est nulle. On calcule plutôt la probabilité de tomber dans une petite zone rectangulaire $dA = dx \cdot dy$ autour de ce point.
Cette probabilité est proportionnelle à l'aire $dA$. La constante de proportionnalité est appelée fonction de densité de probabilité, notée $p(x,y)$.

$$P(\text{dans } dA) \approx p(x,y) \cdot dA$$


2. Les Hypothèses Fondamentales
Nous posons deux principes sur la nature de l'erreur (le hasard du lancer) :
\begin{enumerate}
\item L'indépendance des axes (La simplicité du geste) : L'erreur horizontale ($x$) n'influence pas l'erreur verticale ($y$). La probabilité combinée est donc simplement le produit des probabilités individuelles sur chaque axe :

$$p(x,y) = f(x) \cdot f(y)$$



\item L'isotropie (Le hasard pur) : Le hasard ne privilégie aucune direction. La probabilité dépend uniquement de la distance $r$ au centre, et non de l'angle. D'après le théorème de Pythagore ($r^2 = x^2 + y^2$), il existe une fonction $\phi$ telle que :
...
En égalant ces deux visions de la même probabilité (indépendance et isotropie), nous obtenons notre clé de voûte mathématique :

$$f(x) \cdot f(y) = \phi(x^2 + y^2)$$


4. Résolution Algébrique (L'apparition de l'exponentielle)
Cherchons la forme exacte de la fonction $f$ :
\begin{enumerate}
\item Si l'on se place sur l'axe des abscisses ($y=0$), l'équation devient $f(x) \cdot f(0) = \phi(x^2)$. En posant la constante $\lambda = f(0)$, on déduit que $\phi(x^2) = \lambda f(x)$.
\item Remplaçons $\phi$ dans notre équation de départ : $f(x) \cdot f(y) = \lambda f(\sqrt{x^2 + y^2})$.
\item Pour clarifier, définissons une nouvelle fonction traitant les valeurs au carré : $h(x^2) = \frac{f(x)}{\lambda}$. En posant $a = x^2$ et $b = y^2$, l'équation devient la célèbre équation de Cauchy :

$$h(a) \cdot h(b) = h(a + b)$$



\item La seule solution continue transformant une addition en multiplication est la fonction exponentielle : $h(a) = e^{Aa}$.
\item Retour à notre fonction de départ : $f(x) = \lambda h(x^2) = \lambda e^{Ax^2}$.
\end{enumerate}
Pour que la probabilité diminue lorsqu'on s'éloigne de la cible (l'erreur devient rare), la constante $A$ doit être négative. Posons $A = -k$ (avec $k > 0$). Nous obtenons la structure de la courbe en cloche :

$$f(x) = \lambda e^{-k x^2}$$


5. Normalisation et Paramètres Physiques
La fléchette atterrissant obligatoirement quelque part, l'aire totale sous la courbe de probabilité doit valoir exactement 1.
\begin{enumerate}
\item L'aire totale : L'intégrale de Gauss nous donne $\int_{-\infty}^{\infty} e^{-k x^2} dx = \sqrt{\frac{\pi}{k}}$. Pour que l'aire totale $\lambda \sqrt{\frac{\pi}{k}}$ soit égale à 1, il faut que $\lambda = \sqrt{\frac{k}{\pi}}$.
\item Lien avec la variance ($\sigma^2$) : L'écart-type $\sigma$ mesure l'ampleur moyenne de la dispersion. En calculant la variance $\sigma^2 = \int_{-\infty}^{\infty} x^2 f(x) dx$, on trouve $\sigma^2 = \frac{1}{2k}$. On en déduit que $k = \frac{1}{2\sigma^2}$.
\item Substitution finale : En remplaçant $k$ par $\frac{1}{2\sigma^2}$ dans les expressions de $\lambda$ et de $f(x)$, on obtient l'équation de la loi normale centrée :

$$f(x) = \frac{1}{\sigma\sqrt{2\pi}} e^{ -\frac{x^2}{2\sigma^2} }$$



\item Généralisation : Pour modéliser une distribution centrée sur une autre valeur $\mu$ (la moyenne), il suffit de décaler l'équation en remplaçant $x$ par $(x-\mu)$ :
