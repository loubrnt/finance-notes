...
\begin{intuitionbox}[L'intuition fondamentale : La stratégie de "Cash and Carry"]
...
\begin{enumerate}
    \item À l'origine ($t=0$) : N'ayant pas de capital à mobiliser, on emprunte l'argent à la banque pour acheter l'actif. Le flux de trésorerie initial est parfaitement nul.
    \item Pendant la durée du contrat : On est propriétaire légal de l'action, on encaisse donc ses dividendes réguliers. En parallèle, notre dette à la banque s'alourdit des intérêts au taux sans risque.
    \item À l'échéance ($T$) : On livre l'actif à l'acheteur au prix convenu. Pour qu'il n'y ait pas d'arbitrage (ni gain magique, ni perte), ce prix de vente doit correspondre exactement au montant qu'il nous reste à rembourser à la banque, une fois les dividendes déduits. 
\end{enumerate}
\end{intuitionbox}

\begin{proofbox}[L'argument de non-arbitrage pas à pas]
Traduisons cette intuition mathématiquement pour une action valant $S_0$, avec un taux sans risque $r$ et un taux de dividende continu $d$.
Construisons notre portefeuille à coût zéro aujourd'hui :
\begin{itemize}[label=$\rightarrow$]
    \item Sachant que l'action verse un dividende continu $d$ que nous allons réinvestir en achetant des fractions d'actions, nous n'avons pas besoin d'acheter 1 action entière aujourd'hui. Il suffit d'en acheter une fraction égale à $e^{-dT}$.
    \item Pour financer cet achat, on emprunte exactement la somme nécessaire : $e^{-dT} S_0$.
\end{itemize}
Que se passe-t-il à l'échéance $T$ ?
\begin{itemize}[label=$\rightarrow$]
    \item La fraction d'actif $e^{-dT}$ a généré des dividendes continus. Grâce au réinvestissement, nous possédons désormais exactement $1$ actif complet.
    \item La debt initiale a accumulé des intérêts au taux $r$. Le montant à rembourser est de : $e^{-dT} S_0 \times e^{rT} = S_0 e^{(r-d)T}$.
\end{itemize}
...
\begin{theorembox}[Valeur et Prix théorique d'un contrat Forward]
...
\begin{equation}
    \label{eq:fsec2-forward_theorique}
    F = S_0 e^{(r-d)T}
\end{equation}
...
\begin{equation}
    \label{eq:fsec2-forward_valeur}
    V = e^{-rT} (S_0 e^{(r-d)T} - K)
\end{equation}
\end{theorembox}

\begin{examplebox}[Exemple 1 : Prix théorique vs Valeur en cours]
Considérons une action $S_0 = 100$, avec $r = 0,05$ et $d = 0,02$ sur une durée $T = 1$.

1. Le prix forward juste fixé aujourd'hui :
\begin{equation}
    \label{eq:fsec2-ex_forward_calc}
    F = 100 \times e^{(0,05 - 0,02) \times 1} = 100 \times e^{0,03} \approx 103,05
\end{equation}

2. La valeur d'un contrat en cours :
Imaginons que vous ayez signé un contrat (il y a quelques mois) pour acheter cette action à $K = 100$ dans un an :
\begin{itemize}[label=$\rightarrow$]
    \item À l'échéance, vous paierez $100$ pour un actif dont le prix de livraison équitable aujourd'hui est évalué à $103,05$. Vous faites donc une "bonne affaire" de $3,05$.
    \item La valeur de ce contrat aujourd'hui est ce gain futur actualisé : 
    $V = e^{-0,05} \times (103,05 - 100) \approx 2,90$.
\end{itemize}
\end{examplebox}

\begin{examplebox}[Exemple 2 : Opportunité d'arbitrage en cas de "Mispricing"]
...
Cas d'une surévaluation du marché ($F_{\text{marché}} = 105$) :
Le prix proposé par le marché est trop élevé par rapport au prix théorique d'absence d'arbitrage. Un arbitrageur va exécuter une stratégie de \textit{Cash and Carry} pour capturer un gain certain :
\begin{itemize}[label=$\rightarrow$]
    \item À $t = 0$ (Flux de trésorerie net = $0$) : 
    \begin{itemize}[label=$\cdot$]
        \item Vendre (position courte) le contrat Forward au prix surévalué de $105$ €.
        \item Emprunter la somme de $100 \times e^{-0,02 \times 1} \approx 98,02$ € à la banque au taux sans risque de $5\%$.
        \item Acheter immédiatement la fraction $e^{-0,02}$ d'action au prix spot.
    \end{itemize}
    \item À l'échéance $T = 1$ :
    \begin{itemize}[label=$\cdot$]
        \item Grâce au réinvestissement des dividendes continus, la fraction d'action est devenue exactement $1$ action entière.
        \item Livrer cette action pour honorer le contrat Forward et recevoir les $105$ € du marché.
        \item Rembourser l'emprunt bancaire avec les intérêts accumulés : $98,02 \times e^{0,05 \times 1} \approx 103,05$ €.
    \end{itemize}
    \item Gain d'arbitrage net à $T = 1$ : 
    \begin{equation}
        \label{eq:fsec2-ex_arbitrage_profit}
...
\begin{equation}
    \label{eq:fsec2-parite_call_put}
    C - P = V
\end{equation}
En utilisant la définition de la valeur d'un contrat forward, nous avons :
\begin{equation}
    \label{eq:fsec2-parite_dev}
    C - P = e^{-rT} (S_0 e^{(r-d)T} - K)
\end{equation}
\end{theorembox}

\begin{proofbox}[Justification par les flux finaux (Payoffs)]
Pour démontrer logiquement cette égalité, il suffit de comparer ce que rapportent ces instruments à l'échéance finale $T$ :
\begin{itemize}[label=$\rightarrow$]
    \item Le Call donne le droit d'acheter à $K$, son flux final est $\max(S_T - K, 0)$.
    \item Le Put donne le droit de vendre à $K$, son flux final est $\max(K - S_T, 0)$.
\end{itemize}
...
\begin{theorembox}[Bornes sur le prix d'un Put]
...
\begin{equation}
    \label{eq:fsec2-bornes_put}
    K Z_0 > P_0 > K Z_0 - S_0
\end{equation}
Où $Z_0 = e^{-rT}$ est le prix d'un zéro-coupon.
\end{theorembox}

\begin{proofbox}[Preuve des bornes du Put]
La borne supérieure $P_0 < K Z_0$ découle du fait qu'à l'échéance, le payoff $\max(K - S_T, 0)$ est strictement inférieur ou égal à $K$. À l'origine, le Put doit donc valoir moins que la valeur actualisée de $K$.

Pour la borne inférieure, on utilise la parité Call-Put ($C_0 - P_0 = S_0 - K Z_0$) combinée au fait que $C_0 \ge 0$. En réarrangeant :
\begin{equation}
    \label{eq:fsec2-bornes_put_proof}
    P_0 = C_0 + K Z_0 - S_0 \ge K Z_0 - S_0
\end{equation}
Puisque le prix d'une option est toujours non négatif, on obtient $P_0 \ge \max(K Z_0 - S_0, 0)$.
\end{proofbox}

\begin{theorembox}[Non-optimalité de l'exercice anticipé]
...
\begin{equation}
    \label{eq:fsec2-call_americain}
    C_{\text{américain}} = C_{\text{européen}}
\end{equation}
\end{theorembox}

\begin{intuitionbox}[L'explication par les bornes]
Nous avons démontré que si $r \ge 0$, alors $C_0 > S_0 - K$. 
Si un investisseur décide d'exercer son Call américain immédiatement, il reçoit la valeur intrinsèque immédiate : $S_0 - K$.
Or, en vendant le Call sur le marché, il obtient sa valeur de marché $C_0$, qui est strictement supérieure à $S_0 - K$. L'investisseur a donc toujours intérêt à vendre l'option plutôt qu'à l'exercer. De plus, exercer par anticipation revient à payer le prix d'exercice $K$ immédiatement au lieu de conserver cette trésorerie rémunérée au taux sans risque jusqu'à l'échéance.
\end{intuitionbox}